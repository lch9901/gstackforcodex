...
Use this structure:
1. **Re-ground:** Briefly restate the project, current branch, current design-review pass, and the decision in front of the user.
2. **Simplify:** Explain the design gap in plain English without implementation-heavy jargon.
...
- Treat explicit interaction states, responsive behavior, accessibility, and design-system specificity as cheap wins.
- Distinguish **boilable lakes** from **unbounded oceans**. A lake is a complete design spec for a bounded feature. An ocean is an unscoped rebrand, open-ended design-system rewrite, or product rethinking without constraints.
...
- Deferring obvious empty/error/responsive states without a real reason.
...
history, or review analytics. Do not assume variables from an earlier step still exist.
...
mkdir -p "$_GLOBAL_PROJECT_DIR" "$_LOCAL_GSTACK_DIR" "$_LOCAL_DESIGN_DIR" "$_ANALYTICS_DIR"
echo "ROOT: $_ROOT"
echo "BRANCH: $_BRANCH"
...
* What prior design reviews exist? (check `$HOME/.codex/gstack/analytics/reviews.jsonl` if it exists)
...
### 0D. Focus Areas
Ask directly in chat using the Conversational Decision Protocol: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. Want me to review all 7 dimensions, or focus on specific areas?"

**STOP.** Do NOT proceed until user responds.

If the user chooses a focused review rather than all 7 dimensions:
- confirm exactly which passes are in scope
- run only those passes plus Pass 7 (Unresolved Design Decisions)
- note any skipped passes in the Completion Summary as `SKIPPED BY CHOICE`
- do not silently review skipped passes later
...
5. Ask a direct decision-gate question in chat if there's a genuine design choice to resolve
...
**STOP.** Ask one decision-gate question per issue. Do NOT batch. Recommend + WHY. If no issues, say so and move on. Do NOT proceed until user responds.
...
**STOP.** Ask one decision-gate question per issue. Do NOT batch. Recommend + WHY.
...
```
  ...
  ...
```
Each decision = one direct decision gate in chat with recommendation + WHY + alternatives. Edit the plan with each decision as it's made.

## CRITICAL RULE — How to ask questions
Follow the Conversational Decision Protocol above. Additional rules for plan design reviews:
...
* **Escape hatch:** If a section has no issues, say so and move on. If a gap has an obvious fix, state what you'll add and move on — don't waste a question on it. Only pause for a decision when there is a genuine design choice with meaningful tradeoffs.
...
### TODOS.md updates
After all review passes are complete, present each potential TODO as its own individual decision gate in chat. Never batch TODOs — one per question. Never silently skip this step.
...
Keep an explicit numbered list of every decision gate the user did not answer, explicitly deferred, or interrupted. At the end of the review, display this list under `Unresolved Decisions`. If there are none, say so explicitly. Use this list as the source of truth for the `UNRESOLVED` count in the review log below. Never silently default to an option.
...
_REPO=$(basename "$_ROOT")
_SLUG=$(printf "%s" "$_REPO" | tr '[:upper:]' '[:lower:]' | tr -cs 'a-z0-9' '-')
echo '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","overall_score":OVERALL_SCORE,"unresolved":UNRESOLVED,"decisions_made":DECISIONS_MADE,"commit":"'"$COMMIT"'","repo":"'"$_SLUG"'"}' >> "$HOME/.codex/gstack/analytics/reviews.jsonl"
```
...
- **UNRESOLVED**: number from the explicit `Unresolved Decisions` list
...
After displaying the Review Readiness Dashboard, recommend the next review(s) based on what this design review discovered. Read the local dashboard data to see which reviews have already been run and whether they are stale.

**Recommend /plan-eng-review by default** — it is the required shipping gate. If this design review added significant interaction specifications, new user flows, or changed the information architecture, emphasize that eng review needs to validate the architectural implications. If an eng review already exists but the commit hash shows it predates this design review, note that it may be stale and should be re-run.
