...
Use this structure:
1. **Re-ground:** Briefly restate the project, current branch, current audit/fix phase, and the decision in front of the user.
...
- Treat before/after verification, responsive checks, accessible interaction states, and regression-safe fixes as cheap wins.
- Distinguish **boilable lakes** from **unbounded oceans**. A lake is a complete fix for a bounded design issue or audit scope. An ocean is an unscoped redesign, full rewrite, or indefinite visual research loop.
...
- Deferring obvious responsive/accessibility/UI-state fixes without a real reason.
...
reports, baseline artifacts, or review analytics. Do not assume variables from an earlier
step still exist.
...
_LOCAL_REPORT_DIR="$_LOCAL_GSTACK_DIR/design-reports"
_LOCAL_SCREENSHOT_DIR="$_LOCAL_REPORT_DIR/screenshots"
_GLOBAL_REPORT_DIR="$_GLOBAL_PROJECT_DIR/design-reports"
_ANALYTICS_DIR="$HOME/.codex/gstack/analytics"
_REVIEW_LOG="$_ANALYTICS_DIR/reviews.jsonl"
mkdir -p "$_GLOBAL_PROJECT_DIR" "$_LOCAL_GSTACK_DIR" "$_LOCAL_REPORT_DIR" "$_LOCAL_SCREENSHOT_DIR" "$_GLOBAL_REPORT_DIR" "$_ANALYTICS_DIR"
echo "ROOT: $_ROOT"
echo "BRANCH: $_BRANCH"
echo "GLOBAL_PROJECT_DIR: $_GLOBAL_PROJECT_DIR"
echo "LOCAL_REPORT_DIR: $_LOCAL_REPORT_DIR"
echo "GLOBAL_REPORT_DIR: $_GLOBAL_REPORT_DIR"
echo "REVIEW_LOG: $_REVIEW_LOG"
```

Registry policy:
- `$_GLOBAL_PROJECT_DIR` is the source of truth for cross-session gstack history.
- `$_LOCAL_REPORT_DIR` stores repo-local audit artifacts and screenshots.
- `$_GLOBAL_REPORT_DIR` stores canonical audit reports and baselines for regression history.
...
for diff-aware mode and regression comparisons.
...
4. Set `BASE_BRANCH` to the chosen value and echo it before continuing:
...
**If no URL is given and you're on main/master:** Ask the user for a URL, then stop and wait for their reply before continuing.
...
If `NEEDS_SETUP`:
1. Tell the user browse is unavailable in the current environment.
2. If an equivalent Codex browser or Playwright tool is available, use that instead. For the rest of this skill, treat `$B` as shorthand for that chosen browser automation tool and still produce equivalent screenshots, snapshots, responsive captures, console logs, and perf output.
3. Otherwise stop and tell the user this audit is blocked until browser automation is available.
...
If user picks H → write `.codex/gstack/no-test-bootstrap` and continue without tests.

Before writing the marker file, ensure the directory exists:
```bash
mkdir -p .codex/gstack
```
...
If package installation fails → debug once. If still failing, restore only the bootstrap-touched manifest/config files if it is safe to do so, warn the user, and continue without tests. Never discard unrelated user edits.
...
If tests fail → debug once. If still failing, restore only bootstrap-touched manifest/config files if it is safe to do so, warn the user, and continue without tests. Never discard unrelated user edits.
...
```bash
REPORT_DIR="$_ROOT/.codex/gstack/design-reports"
...
1. Analyze the branch diff: `git diff "$BASE_BRANCH"...HEAD --name-only`
2. Map changed files to affected pages/routes
3. Detect running app on common local ports (3000, 4000, 8080)
4. Audit only affected pages, compare design quality before/after

### Regression (`--regression` or previous `design-baseline.json` found)
Run full audit, then load the previous baseline in this order:
1. `$_GLOBAL_REPORT_DIR/${_BRANCH_SAFE}-design-baseline.json`
2. `$_LOCAL_REPORT_DIR/design-baseline.json`
Compare: per-category grade deltas, new findings, resolved findings. Output regression table in report.
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: ask directly in chat: "This site requires authentication. Want to import cookies from your browser? Run `/setup-browser-cookies` first if needed." Stop and wait for the user's reply before continuing.
...
**Local:** `$_ROOT/.codex/gstack/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
mkdir -p "$_GLOBAL_REPORT_DIR"
```
Write to: `$_GLOBAL_REPORT_DIR/${_BRANCH_SAFE}-design-audit-{datetime}.md`

**Baseline:** Write both:
- `$_LOCAL_REPORT_DIR/design-baseline.json`
- `$_GLOBAL_REPORT_DIR/${_BRANCH_SAFE}-design-baseline.json`
...
When a previous baseline exists or `--regression` flag is used:
