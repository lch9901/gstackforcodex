...
Use this structure:
1. **Re-ground:** Briefly restate the project, current branch, current docs phase, and the decision in front of the user.
...
- Treat factual doc sync, cross-links, release note hygiene, and consistency checks as cheap wins.
- Distinguish **boilable lakes** from **unbounded oceans**. A lake is a bounded doc sync for one branch. An ocean is a branding rewrite, new docs IA, or speculative documentation overhaul.
...
- Leaving factual drift in place because the prose “mostly works.”
...
or repo-local documentation notes. Do not assume variables from an earlier step still exist.
...
_LOCAL_DOCS_DIR="$_LOCAL_GSTACK_DIR/docs"
_GLOBAL_DOCS_DIR="$_GLOBAL_PROJECT_DIR/docs"
...
mkdir -p "$_GLOBAL_PROJECT_DIR" "$_LOCAL_GSTACK_DIR" "$_LOCAL_DOCS_DIR" "$_GLOBAL_DOCS_DIR" "$_ANALYTICS_DIR"
echo "ROOT: $_ROOT"
echo "BRANCH: $_BRANCH"
echo "GLOBAL_PROJECT_DIR: $_GLOBAL_PROJECT_DIR"
echo "LOCAL_DOCS_DIR: $_LOCAL_DOCS_DIR"
echo "GLOBAL_DOCS_DIR: $_GLOBAL_DOCS_DIR"
...
- `$_LOCAL_DOCS_DIR` stores repo-local doc-sync notes if this workflow writes any.
- `$_GLOBAL_DOCS_DIR` stores canonical doc-sync summaries for cross-session context.
...
- Bump VERSION without asking — always use a direct chat decision gate for version changes
- Overwrite CHANGELOG wholesale — use precise line-anchored edits only
...
```bash
find . -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.codex/*" -not -path "./.context/*" | sort
```
...
For each risky or questionable update identified in Step 2, ask directly in chat using the Conversational Decision Protocol with:
...
3. Never regenerate a CHANGELOG entry from scratch. The entry was written by `/ship` from the
   ...
   rewriting history.
4. If an entry looks wrong or incomplete, ask directly in chat — do NOT silently fix it.
5. Use precise line-anchored edits — never overwrite CHANGELOG.md wholesale.
...
- Auto-fix minor voice adjustments. Use a direct chat decision gate if a rewrite would alter meaning.
...
5. Flag any contradictions between documents. Auto-fix clear factual inconsistencies (e.g., a
   version mismatch). Use a direct chat decision gate for narrative contradictions.

---

## Step 7: TODOS.md Cleanup

This is a second pass that complements `/ship`'s Step 5.5. Read `$_SUPPORT_REVIEW_DIR/TODOS-format.md` (if
...
2. **Items needing description updates:** If a TODO references files or components that were
   significantly changed, its description may be stale. Ask directly in chat to confirm whether
   the TODO should be updated, completed, or left as-is.

3. **New deferred work:** Check the diff for `TODO`, `FIXME`, `HACK`, and `XXX` comments. For
   each one that represents meaningful deferred work (not a trivial inline note), ask directly
   in chat whether it should be captured in TODOS.md.
...
```bash
git diff "$BASE_REF"...HEAD -- VERSION
```

3. **If VERSION was NOT bumped:** Ask directly in chat:
...
4. **If VERSION was already bumped:** Do NOT skip silently. Instead, check whether the bump
   ...
   b. Read the full diff (`git diff "$BASE_REF"...HEAD --stat` and `git diff "$BASE_REF"...HEAD --name-only`).
      Are there significant changes (new features, new skills, new commands, major refactors)
      that are NOT mentioned in the CHANGELOG entry for the current version?
   c. **If the CHANGELOG entry covers everything:** Skip — output "VERSION: Already bumped to
      vX.Y.Z, covers all changes."
   d. **If there are significant uncovered changes:** Ask directly in chat explaining what the
...
1. If `gh` is available and authenticated, read the existing PR body into a PID-unique tempfile. Otherwise skip the rest of the PR body update subsection.
