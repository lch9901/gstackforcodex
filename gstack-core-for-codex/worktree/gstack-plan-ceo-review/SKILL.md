...
telemetry prompts, upgrade flows, and host-only interaction APIs.
...
Use this structure:
1. **Re-ground:** Briefly restate the project, current branch, current mode, and where you are in the review.
2. **Simplify:** Explain the decision in plain English without implementation jargon.
3. **Recommend:** Give a clear recommendation, including completeness posture when helpful.
4. **Ask one question:** Offer 2-3 concrete options only if there is a real tradeoff.
...
- Treat explicit error handling, documentation, diagrams, and edge-case coverage as cheap wins.
- Distinguish **boilable lakes** from **unbounded oceans**. A lake is a fully specified plan or complete review. An ocean is a rewrite, platform migration, or speculative expansion with unclear bounds.
...
Re-run this bootstrap block before any phase that relies on repository paths, the
global gstack registry, or review analytics. Do not assume variables from an earlier
step still exist.
...
_LOCAL_DESIGN_DIR="$_LOCAL_GSTACK_DIR/designs"
_GLOBAL_CEO_DIR="$_GLOBAL_PROJECT_DIR/ceo-plans"
_LOCAL_CEO_DIR="$_LOCAL_GSTACK_DIR/ceo-plans"
_ANALYTICS_DIR="$HOME/.codex/gstack/analytics"
_REVIEW_LOG="$_ANALYTICS_DIR/reviews.jsonl"
mkdir -p "$_GLOBAL_PROJECT_DIR" "$_LOCAL_GSTACK_DIR" "$_LOCAL_DESIGN_DIR" "$_GLOBAL_CEO_DIR" "$_LOCAL_CEO_DIR" "$_ANALYTICS_DIR"
echo "ROOT: $_ROOT"
echo "BRANCH: $_BRANCH"
echo "GLOBAL_PROJECT_DIR: $_GLOBAL_PROJECT_DIR"
echo "GLOBAL_CEO_DIR: $_GLOBAL_CEO_DIR"
echo "LOCAL_CEO_DIR: $_LOCAL_CEO_DIR"
echo "REVIEW_LOG: $_REVIEW_LOG"
```

Registry policy:
- `$_GLOBAL_PROJECT_DIR` is the source of truth for cross-session gstack history.
- `$_GLOBAL_CEO_DIR` is the canonical store for CEO plans.
- `$_LOCAL_CEO_DIR` mirrors canonical plans into the current repo for local visibility and downstream Codex workflows.
- `$_LOCAL_DESIGN_DIR` mirrors related design docs inside the repo.
...
3. If GitHub CLI is unavailable or not authenticated, try the remote default branch:
   `git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's@^refs/remotes/origin/@@'`

4. If that fails, check common local defaults in this order:
   `main`, `master`, `trunk`, `develop`

5. If all detection fails, fall back to `main`.
...
* SCOPE EXPANSION: You are building a cathedral. Envision the platonic ideal. Push scope UP. Ask "what would make this 10x better for 2x the effort?" You have permission to dream — and to recommend enthusiastically. But every expansion is the user's decision. Present each scope-expanding idea as its own decision gate in chat. The user opts in or out.
* SELECTIVE EXPANSION: You are a rigorous reviewer who also has taste. Hold the current scope as your baseline — make it bulletproof. But separately, surface every expansion opportunity you see and present each one individually as its own decision gate in chat so the user can cherry-pick. Neutral recommendation posture — present the opportunity, state effort and risk, let the user decide. Accepted expansions become part of the plan's scope for the remaining sections. Rejected ones go to "NOT in scope."
...
Critical rule: In ALL modes, the user is 100% in control. Every scope change is an explicit opt-in through direct chat — never silently add or remove scope. Once the user selects a mode, COMMIT to it. Do not silently drift toward a different mode. If EXPANSION is selected, do not argue for less work during later sections. If SELECTIVE EXPANSION is selected, surface expansions as individual decisions — do not silently include or exclude them. If REDUCTION is selected, do not sneak scope back in. Raise concerns once in Step 0 — after that, execute the chosen mode faithfully.
...
_BRANCH_SAFE=$(printf "%s" "$_BRANCH" | tr '/:' '--')
_GLOBAL_PROJECT_DIR="$HOME/.codex/gstack/projects/$_SLUG"
DESIGN=$(find "$_GLOBAL_PROJECT_DIR" -maxdepth 1 -type f -name "*-${_BRANCH_SAFE}-design-*.md" | sort | tail -1)
[ -z "$DESIGN" ] && DESIGN=$(find "$_GLOBAL_PROJECT_DIR" -maxdepth 1 -type f -name "*-design-*.md" | sort | tail -1)
...
### 0F. Mode Selection
Run this immediately after 0C-bis. Do not start 0D, 0D-POST, or 0E until the user has
approved both the implementation approach and the review mode.
...
**STOP.** Ask one decision-gate question per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.

### 0D. Mode-Specific Analysis
Run this only after 0F is complete.
...
4. **Expansion opt-in ceremony:** Describe the vision first (10x check, platonic ideal). Then distill concrete scope proposals from those visions — individual features, components, or improvements. Present each proposal as its own decision gate in chat. Recommend enthusiastically — explain why it's worth doing. But the user decides. Options: **A)** Add to this plan's scope **B)** Defer to TODOS.md **C)** Skip. Accepted items become plan scope for all remaining review sections. Rejected items go to "NOT in scope."
...
4. **Cherry-pick ceremony:** Present each expansion opportunity as its own individual decision gate in chat. Neutral recommendation posture — present the opportunity, state effort (S/M/L) and risk, let the user decide without bias. Options: **A)** Add to this plan's scope **B)** Defer to TODOS.md **C)** Skip. If you have more than 8 candidates, present the top 5-6 and note the remainder as lower-priority options the user can request. Accepted items become plan scope for all remaining review sections. Rejected items go to "NOT in scope."
...
_BRANCH_SAFE=$(printf "%s" "$_BRANCH" | tr '/:' '--')
_GLOBAL_PROJECT_DIR="$HOME/.codex/gstack/projects/$_SLUG"
_LOCAL_CEO_DIR="$_ROOT/.codex/gstack/ceo-plans"
_GLOBAL_CEO_DIR="$_GLOBAL_PROJECT_DIR/ceo-plans"
mkdir -p "$_GLOBAL_CEO_DIR" "$_LOCAL_CEO_DIR"
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p "$_GLOBAL_CEO_DIR/archive"
# For each stale plan: mv "$_GLOBAL_CEO_DIR/{old-plan}.md" "$_GLOBAL_CEO_DIR/archive/"
```

Use a canonical global path plus a repo-local mirror:

```bash
DATE=$(date +%F)
DATETIME=$(date +%Y%m%d-%H%M%S)
FEATURE_SLUG="{feature-slug}"
CEO_DOC_BASENAME="${DATE}-${_BRANCH_SAFE}-ceo-${FEATURE_SLUG}-${DATETIME}.md"
CEO_DOC_PATH="$_GLOBAL_CEO_DIR/$CEO_DOC_BASENAME"
CEO_LOCAL_MIRROR_PATH="$_LOCAL_CEO_DIR/$CEO_DOC_BASENAME"
echo "Canonical CEO plan: $CEO_DOC_PATH"
echo "Repo mirror: $CEO_LOCAL_MIRROR_PATH"
```

Write the canonical file first using this format:
...
After writing the canonical file, mirror it into the repo:

```bash
cp "$CEO_DOC_PATH" "$CEO_LOCAL_MIRROR_PATH"
```

After each subsequent edit to the canonical file, refresh the mirror again so downstream
skills see the same version.
...
**Step 1: Prefer an independent reviewer**

Preserve the original adversarial-review intent whenever the host allows it.

- If the current host supports an independent reviewer with separate context and the user has explicitly allowed delegation, use that path.
- Otherwise, run a blind second-pass review yourself: re-read only the document on disk, ignore the surrounding conversation, and review it as if you were seeing it for the first time.
- If you are using the fallback, tell the user: "Independent reviewer unavailable in this host, so I ran a blind second-pass review instead."

Prompt the independent reviewer, or use the same rubric for the blind second-pass, with:
...
2. Re-run the same review mode on the updated document
...
If the reviewer fails, times out, or is unavailable — skip the review loop entirely.
...
3. Append metrics:
```bash
mkdir -p "$HOME/.codex/gstack/analytics"
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE,"canonical_path":"'"$CEO_DOC_PATH"'"}' >> "$HOME/.codex/gstack/analytics/spec-review.jsonl" 2>/dev/null || true
```
Replace ITERATIONS, FOUND, FIXED, REMAINING, SCORE with actual values from the review.

### 0E. Temporal Interrogation (EXPANSION, SELECTIVE EXPANSION, and HOLD modes)
Run this after the selected mode's 0D analysis is complete, and after 0D-POST for
EXPANSION / SELECTIVE EXPANSION if a CEO plan was written.
...
Follow the Conversational Decision Protocol above. Additional rules for plan reviews:
* **One issue = one decision gate.** Never combine multiple issues into one question.
* Describe the problem concretely, with file and line references.
* Present 2-3 options, including "do nothing" where reasonable.
* For each option: effort, risk, and maintenance burden in one line.
* **Map the reasoning to my engineering preferences above.** One sentence connecting your recommendation to a specific preference.
* Label with issue NUMBER + option LETTER (e.g., "3A", "3B").
* **Escape hatch:** If a section has no issues, say so and move on. If an issue has an obvious fix with no real alternatives, state what you'll do and move on — don't waste a question on it. Only pause for a decision when there is a genuine tradeoff.
...
### TODOS.md updates
Present each potential TODO as its own individual decision gate in chat. Never batch TODOs — one per question. Never silently skip this step. Use the same structure gstack expects: What, Why, Pros, Cons, Context, Effort estimate, Priority, and Depends on / blocked by.
...
If any decision gate goes unanswered, note it here. Never silently default.

## Review Log

After producing the Completion Summary above, persist the review result in the local
Codex analytics registry.
...
_REPO=$(basename "$_ROOT")
_SLUG=$(printf "%s" "$_REPO" | tr '[:upper:]' '[:lower:]' | tr -cs 'a-z0-9' '-')
echo '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":UNRESOLVED,"critical_gaps":CRITICAL_GAPS,"mode":"MODE","commit":"'"$COMMIT"'","repo":"'"$_SLUG"'"}' >> "$HOME/.codex/gstack/analytics/reviews.jsonl"
```

Before writing this line, substitute:
- **STATUS**: `clean` if 0 unresolved decisions and 0 critical gaps; otherwise `issues_open`
- **UNRESOLVED**: number from "Unresolved decisions"
- **CRITICAL_GAPS**: number from "Failure modes: ___ CRITICAL GAPS"
- **MODE**: the mode the user selected (`SCOPE_EXPANSION`, `SELECTIVE_EXPANSION`, `HOLD_SCOPE`, `SCOPE_REDUCTION`)

## Review Readiness Dashboard

After completing the review, read `"$HOME/.codex/gstack/analytics/reviews.jsonl"` if it
exists and build a lightweight dashboard from the most recent entry for each skill
within the last 7 days. If the log does not exist, state that no prior review data is
available yet.

Track these skills when present:
- `plan-eng-review`
- `plan-ceo-review`
- `plan-design-review`
- `design-review-lite`
...
For Design Review, show whichever is more recent between `plan-design-review` and
`design-review-lite`, appending `(FULL)` or `(LITE)` to the status. Display:
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, and performance.
...
- **Codex Review (optional):** Independent review and adversarial challenge if your Codex environment or team workflow provides one.
...
- Determine the current HEAD commit hash with `git rev-parse --short HEAD`
- For each review entry that has a `commit` field: compare it against current HEAD. If different, count elapsed commits with `git rev-list --count STORED_COMMIT..HEAD`. Display: "Note: {skill} review from {date} may be stale — {N} commits since review"
- For entries without a `commit` field: display "Note: {skill} review from {date} has no commit tracking — consider re-running for accurate staleness detection"
- If all reviews match the current HEAD, do not display any staleness notes

## Next Steps — Review Chaining

After displaying the Review Readiness Dashboard, recommend the next review(s) based on
what this CEO review discovered. Read the local dashboard data to see which reviews have
already been run and whether they are stale.

**Recommend /plan-eng-review by default** — it is the required shipping gate. If this
CEO review expanded scope, changed architectural direction, or accepted scope
expansions, emphasize that a fresh eng review is needed. If an eng review already
exists in the dashboard but the commit hash shows it predates this CEO review, note
that it may be stale and should be re-run.
...
Ask directly in chat using the Conversational Decision Protocol. Include only applicable options:
...
At the end of the review, if the vision produced a compelling feature direction, offer to promote the CEO plan to the project repo. Ask directly in chat using the Conversational Decision Protocol:
...
- **B)** Keep it only in the global gstack registry and repo-local mirror
- **C)** Skip

If promoted, use the canonical CEO plan from 0D-POST. If `CEO_DOC_PATH` is not still in
scope, re-bootstrap and locate the newest matching plan for the current branch under
`$_GLOBAL_CEO_DIR`. Then:
...
_GLOBAL_PROJECT_DIR="$HOME/.codex/gstack/projects/$_SLUG"
_GLOBAL_CEO_DIR="$_GLOBAL_PROJECT_DIR/ceo-plans"
CEO_DOC_PATH=$(find "$_GLOBAL_CEO_DIR" -maxdepth 1 -type f -name "*-${_BRANCH_SAFE}-ceo-*.md" | sort | tail -1)
[ -z "$CEO_DOC_PATH" ] && echo "No matching CEO plan found for current branch" && exit 1
CEO_DOC_BASENAME=$(basename "$CEO_DOC_PATH")
CEO_LOCAL_MIRROR_PATH="$_ROOT/.codex/gstack/ceo-plans/$CEO_DOC_BASENAME"
FEATURE_DOC_SLUG="{feature-slug}"
[ "$FEATURE_DOC_SLUG" = "{feature-slug}" ] && FEATURE_DOC_SLUG=$(printf "%s" "$CEO_DOC_BASENAME" | sed -E 's/^[0-9]{4}-[0-9]{2}-[0-9]{2}-.*-ceo-(.*)-[0-9]{8}-[0-9]{6}\.md$/\1/')
[ -z "$FEATURE_DOC_SLUG" ] && FEATURE_DOC_SLUG="ceo-plan"
mkdir -p "$_ROOT/docs/designs"
cp "$CEO_DOC_PATH" "$_ROOT/docs/designs/$FEATURE_DOC_SLUG.md"
perl -0pi -e 's/^status: ACTIVE$/status: PROMOTED/m' "$CEO_DOC_PATH"
mkdir -p "$(dirname "$CEO_LOCAL_MIRROR_PATH")"
cp "$CEO_DOC_PATH" "$CEO_LOCAL_MIRROR_PATH"
```

If the canonical plan cannot be found, stop and tell the user the promotion could not
be completed. Do not continue with a placeholder path.
